...
@interface MTRecentViewController ()<MTDealCellDelegate>
...
- (void)remove {
    ...
    for (MTDeal *deal in self.deals) {
        if (deal.isChecking) {
            [MTDatabaseTool deleteRecentDeal:deal];
...
- (void)loadMoreDeals {
    ...
    //加载数据
    [self.deals addObjectsFromArray:[MTDatabaseTool recentDeals:self.currentPage]];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    //是否隐藏上拉加载控件
    int collectCount = [MTDatabaseTool recentDealsCount];
